...
class ProductServiceImplTest {
  ...
  void testGetAllProducts() {
    List<Product> products =
        List.of(new Product(1L, "Producto A", 100L, 0), new Product(2L, "Producto B", 200L, 0));
  ...
  void getProductById_ShouldThrowInvalidProductException_WhenNotFound() {
    ...
    InvalidProductException ex =
        assertThrows(InvalidProductException.class, () -> productService.getProductById(99L));
  ...
  void search_ShouldReturnPage() {
    ...
    criteria.setPage(0);
    criteria.setSize(5);
    List<Product> list = List.of(new Product(1L, "X1", 10L, 0), new Product(2L, "X2", 20L, 0));
  ...
  void updateProduct_ShouldThrowInvalidProductException_WhenNotExists() {
    ...
    InvalidProductException ex =
        assertThrows(InvalidProductException.class, () -> productService.updateProduct(id, req));
    assertEquals(PRODUCT_NOT_FOUND, ex.getMessage());
    verify(productRepository).findById(id);
  }
  ...
  void deleteProduct_ShouldThrowInvalidProductException_WhenNotExists() {
    ...
    InvalidProductException ex =
        assertThrows(InvalidProductException.class, () -> productService.deleteProduct(id));
    assertEquals(PRODUCT_NOT_FOUND, ex.getMessage());
    verify(productRepository).existsById(id);
