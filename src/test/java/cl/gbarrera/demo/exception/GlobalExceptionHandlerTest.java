...
class GlobalExceptionHandlerTest {
  ...
  void handleInvalidProductException_shouldReturnExpectedResponse() {
    InvalidProductException ex =
        new InvalidProductException("Producto inválido", HttpStatus.NOT_FOUND);
  ...
  void handleValidationException_shouldReturnBadRequest() {
    ...
    assertEquals(
        MALFORMED_JSON_OR_FIELD_TYPES, Objects.requireNonNull(response.getBody()).getError());
    assertEquals("/api/test", response.getBody().getPath());
    assertNotNull(response.getBody().getRequestId());
    assertNotNull(response.getBody().getTimestamp());
  }
  ...
  void handleOptimisticLockException_shouldReturnConflict() {
    ...
    final String EXPECTED_ERROR_MESSAGE =
